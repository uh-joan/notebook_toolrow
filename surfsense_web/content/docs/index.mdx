---
title: Prerequisites
description: Required setup's before setting up SourceBook
full: true
---

## PGVector installation Guide 

SourceBook requires the pgvector extension for PostgreSQL:
...
SourceBook supports both Google OAuth and local email/password authentication. Google OAuth is optional - if you prefer local authentication, you can skip this section.

**Note**: Google OAuth setup is **required** in your `.env` files if you want to use the Gmail and Google Calendar connectors in SourceBook.
...
SourceBook supports three ETL (Extract, Transform, Load) services for converting files to LLM-friendly formats:
...
This is not required for SourceBook to work. But it is always a good idea to monitor LLM interactions. So we do not have those WTH moments.

1. Get a LangSmith API key from [smith.langchain.com](https://smith.langchain.com/)
2. This helps in observing SourceBook Researcher Agent.
...
SourceBook have 2 options for saving webpages:
- [SourceBook Extension](https://github.com/MODSetter/SourceBook/tree/main/surfsense_browser_extension) (Overall better experience & ability to save private webpages, recommended)
- Crawler (If you want to save public webpages)

**NOTE:** SourceBook currently uses [Firecrawl.py](https://www.firecrawl.dev/) for web crawling. If you plan on using the crawler, you will need to create a Firecrawl account and get an API key.
...
SourceBook provides two powerful AI agents to help you research and discover information:

### SourceBook Researcher
...
**Try:** "What year was semaglutide approved in the US?" • "Summarize key label warnings in these PDFs" • "Compare endpoints across these trials"

### SourceBook Discover
Fetch live sources via Toolrow MCP (FDA, ct.gov, PubMed, WHO, SEC, codes)—auto-cited with one-click Add to Sources.

**Try:** "ICD-10 & MeSH for obesity" • "US FDA drugs for T2D" • "Phase 2/3 trials recruiting in EU" • "Latest PubMed reviews on GLP-1"

---

## Next Steps

Once you have all prerequisites in place, proceed to the [installation guide](/docs/installation) to set up SourceBook.